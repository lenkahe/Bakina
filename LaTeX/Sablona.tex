...
\Autor{Lenka Heldová}{Lenka Heldová}

\NazevPrace{Analýza a návrh algoritmů pro optimalizaci vizualizace dat}{Analýza a návrh algoritmů pro optimalizaci vizualizace dat}{Analysis and design of algorithms to optimize data visualization}
...
\VedouciPraceSTituly{Mgr. Jiří Zelinka, Dr.} %% Neni potreba pro rigorozni prace

\StudijniProgram{Matematika}{Mathematics}

\StudijniObor{Modelování a výpočty}{Modelling and Calculations}

\PocetStran{ix\,$+$\,45}

\KlicovaSlova{PerfCake; testování softvéru; optimalizace; grafy; jadrové odhady; směrodatná odchylka; Nadaraya-Watsonove odhady; redukční algoritmy}{Perfcake; software testing; optimalization; graphs; kernel estimations; standard deviation; Nadaraya-Watson estimations; reduction algorithms}
...
{In this bachelor thesis we design algorithms to optimize vizualization of data, which are output of the testing tool - PerfCake. These algorithms are composed of two parts, smoothing and reduction. The kernel estimation, that is used for smoothing part, and three diffrent reduction algorithms are introduced in fifth chapter. All three reduction algorithms are afterwards compared in the last, sixth chapter. They are compared in few different aspects, the best algorithm is suitable for real use to optimize data visualization in the PerfCake tool.}

\TextPodekovani%
{Na tomto mieste by som chcela poďakovať vedúcemu bakalárskej práce Mgr. Jiřímu Zelinkovi,
Dr. a odborným konzultantom Mgr. Martinovi Večeřovi a Ing. Pavlovi Macíkovi za veľkú pomoc, dôkladné korektúry, odborné rady, inšpiráciu a hlavne za čas, ktorý mi pri písaní tejto práce venovali. Veľká vďaka patrí tiež mojej rodine a priateľom,
ktorí ma počas štúdia podporovali.
}
...
% 
 \NazevPraceSLOVENSKY{Analýza a návrh algoritmov pre optimalizáciu vizualizácie dát}
% 
\NazevUstavuSLOVENSKY{Ústav matematiky a štatistiky}
% 
\StudijniProgramSLOVENSKY{Matematika}
% 
\StudijniOborSLOVENSKY{Modelovanie a výpočty}
% 
\KlicovaSlovaSLOVENSKY{PerfCake; testovanie softvéru; optimalizácia; grafy; jadrové odhady; smerodajná odchýlka; Nadaraya-Watsonove odhady; redukčné algoritmy}
% 
\AbstraktSLOVENSKY{V tejto bakalárskej práci sa venujeme návrhu algoritmov pre optimalizáciu vizualizácie dát, ktoré sú výstupom nástroja na testovanie výkonu PerfCake. V práci sú opísané dve časti algoritmov, vyhladzovacia a redukčná. Pri vyhladzovaní sú používané jadrové odhady, pre redukčnú časť sú predstavené tri rôzne algoritmy, ktoré sú na konci práce porovnané. Podľa niekoľkých aspektov je zvolený najlepší, reálne použiteľný algoritmus na optimalizáciu dát, pre vykreslenie do grafov.}
% 
...
\vloz{text_prace/08_Kapitola_06}
\cleardoublepage

\HlavickaZaver
% \pdfbookmark{Závěr}{Zaver}
\vloz{text_prace/09_Zaver}
\cleardoublepage
